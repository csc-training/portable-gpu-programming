...
2. The code run quite much longer than the previous code without norm calculation (1.9 s).
   ...
   and OpenMP tasks. We can easily afford extra threads on LUMI. Every LUMI-G node
   has 7 CPU cores per a MI250X GCD. Take a second thread in to use with
   `--cpus-per-task=2` flag:

       srun -p dev-g --nodes=1 --ntasks-per-node=1 --cpus-per-task=2 --gpus-per-node=1 -t 0:10:00 ./poisson.x 4096 5000 3
