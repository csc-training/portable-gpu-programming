...
1. Running the provided code without changes with default settings:
       ...
       export HSA_XNACK=1
       srun -p dev-g --nodes=1 --ntasks-per-node=1 --cpus-per-task=2 --gpus-per-node=1 -t 0:10:00 ./poisson.x
   ...
   We have attempted to increase the performance by making sure that the memory allocations are aligned,
   but that doesn't seem to make a difference here.

   Overlapping the I/O step with computation would also need some more code as now
