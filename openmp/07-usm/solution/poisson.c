...
void run(const int n, const int niter, const double normmax)
{
    ...
    size_t nbytes = n2 * sizeof(double);
    nbytes = (nbytes % align) ? (nbytes + align - nbytes % align) : nbytes;

    double h2, *f, *u, *unew;
    f = (double*)aligned_alloc(align, nbytes);
    u = (double*)aligned_alloc(align, nbytes);
    unew = (double*)aligned_alloc(align, nbytes);
    h2 = 1.0;
...
#pragma omp single
{
    ...
    for (int it = 1; it < niter + 1; it++) {
        ...
        if (it % 100 == 0) {
            double norm2 = 0.0;
            #pragma omp target map(tofrom: norm2)  // map needed here for correctness
