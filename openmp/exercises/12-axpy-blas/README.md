...
1. The code `axpy` tries to call axpy from cuBLAS/hipBLAS, but it's broken.
   ...
   On Roihu (NVIDIA), the code can be compiled with
       ...
       nvc -mp=gpu -O3 -gpu=cc90 -Wall -DCUDA axpy.c -L${NVHPC_ROOT}/math_libs/13.1/targets/sbsa-linux/lib -lcublas -o axpy.x
       nvfortran -mp=gpu -O3 -gpu=cc90 -DCUDA axpy.F90 -L${NVHPC_ROOT}/math_libs/13.1/targets/sbsa-linux/lib -lcublas -o axpy.x
